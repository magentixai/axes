...
| O-4 | **Reachability vs crypto outcomes:** do not map TCP timeout / WAF ban / vantage failure to `unsigned` or `signature-invalid` in sweeps or assurance language. | x402 MECHANISM 1.1.1; coherence row 5 | Conformance / reportability guidance | Applies to any AXES online verify or aggregator sweep. |
...
- [ ] Coherence tracker rows updated when each item closes.
