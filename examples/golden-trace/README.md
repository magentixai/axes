...
**Stubbed and disclosed as such:** envelope signatures (`SIG-STUB`); the external anchor store (`anchoring_method: "write_once_store (SIMULATED)"`, demo `anchor_store_ref`); webhook mTLS authenticity of pacs.002. The reports disclose every stub - practising the scoped-assurance language the standard mandates. Structured `basis_status` (`demonstrated` | `stubbed` | `simulated`) is specified in Module 01 so a verifier can reach those disclosures without parsing a parenthetical; corpus values land in the announced gt-v2.1 regeneration (WO16 Task 12 / Task 18).

**Illustrative stored latency (WO16 Task 4b).** At the verified corpus tag `corpus/2026-08-08-gt-v2` the anchor envelope carries `anchoring_latency_ms: 740` with `occurred_at` 09:05:00.000Z, `emitted_at` 09:05:00.400Z, `recorded_at` 09:05:00.900Z, `anchored_at` 09:05:00.000Z. 740 reconciles with no pair of those timestamps, and `anchored_at` precedes emission of the record whose chain head it anchors. The value is disclosed here as an illustrative defect, in the same register as `SIG-STUB` and SIMULATED, until the announced regeneration replaces the stored derived field with `anchor_requested_at` and a named derivation. Do not treat 740 as a measurement.
