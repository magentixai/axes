...
| D-017 | **Field identifiers MUST NOT bake a unit into the key** (bar Rule 3). Rename `anchoring_latency_ms` → `anchoring_latency`; the unit is milliseconds and is declared in the field definition / catalogue, not the identifier. Same class of defect as external proposals that encode `Hours` / `_ms` in names. Schema unfrozen - fixed before raising the rule publicly. Other `_ms` / `_s` keys (e.g. `clock_skew_ms`, `declared_heartbeat_interval_s`) are tracked for the same scrub, not renamed in this decision | decided 2026-08-17 | GAP-TECH-001 field list; applied to the then-default-branch corpus (retired tag `corpus/2026-08-15-pre-merge`). The verified corpus of record (`corpus/2026-08-08-gt-v2`) is unchanged until the announced gt-v2.1 release (D-018, WO16 Task 12) |
| D-018 | **The corpus changes only in announced releases.** No regeneration on the default branch outside a tagged release with a `RELEASES.md` row, published digests, and notice to anyone who published a verification result against the previous release. A rename may be decided any time; it lands in the corpus at the next release. Corpus of record until gt-v2.1: tag `corpus/2026-08-08-gt-v2` (`776cc0b`) | decided 2026-08-17 | docs/17; RELEASES.md; VERIFY.md |
...
|---|---|---|---|
| P1-1 | Canonicalisation incl. redaction-tolerant hashing (salted per-field vs Merkle-structured) | **open - active spike** - [#5](https://github.com/magentixai/axes/issues/5) | Golden Trace is the test corpus; JCS (RFC 8785) baseline supported by steward prior art ([CrossMsg-Signing](https://github.com/magentixai/CrossMsg-Signing) - tested over ISO 20022 content); spike focuses on the commitment half. Exclusion-at-signing ≠ redaction-after-signing (docs/09). GAP-EA-001 / TRK-005. Vectors seed: [#6](https://github.com/magentixai/axes/issues/6) (pinned) |
...
| Derivation Profile Registry as the mechanism for the derived layer's traceability claim | accept-core | observation-versus-derivation principle, 15 Aug 2026; docs/01 §4 |
...
| Derived values are not stored unless operative (computed and acted upon by the executing system) | accept-core | observation-versus-derivation principle; carve-out: store if the executing system computed and acted on the value |
| SE-C4 conformance claims must name a derivation profile | accept-conditional on DPR-002/003 | docs/07; DPR-011 |
| Field keys are `lower_snake`; enum values are `lower_snake` | accept-core | ISO 20022 API/JSON best-practices whitepaper §7.1 recommends unabbreviated snake_case for JSON representations of ISO 20022 semantics; lowerCamelCase appears in no ISO 20022 representation. AXES is internally consistent with zero camelCase keys |
| Concept-level interoperability via declared representation pairs, never derived transforms | accept-core | camel-to-snake transforms are ambiguous at digit boundaries and acronyms. ISO 20022 stores abbreviations in the repository at design time rather than deriving them. Under JCS two spellings are two content identities |
| Canonicalisation registry value is `RFC8785-JCS`; informal `RFC8785` is retired | accept-core | corpus and vectors at `776cc0b`; Colin H Winter (MarkovianProtocol) asked twice in axes#6 (7 and 8 Aug 2026). Identifier is an immutable dereference (docs/09); property change requires a new identifier |
...
| 2026-08-17 | Canonicalisation registry identifier `RFC8785-JCS` | Adopt as normative dereferenceable statement (sort, normalisation, digest encoding) | Question posed twice by MarkovianProtocol in axes#6; corpus already shipped this form | axes#6; docs/09; P1-1 | Adopted |
